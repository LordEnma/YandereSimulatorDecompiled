...
public class AlarmDiscScript : MonoBehaviour
{
			...
			if (Student != null && Student.enabled && Student.DistractionSpot != new Vector3(base.transform.position.x, Student.transform.position.y, base.transform.position.z))
			{
				...
				if (!Radio)
				{
					if (Student != Originator)
					{
						...
						if ((!Student.TurnOffRadio && Student.Alive && !Student.Blind && !Student.Pushed && !Student.Dying && !Student.Alarmed && !Student.Guarding && !Student.Wet && !Student.Slave && !Student.CheckingNote && !Student.WitnessedMurder && !Student.WitnessedCorpse && !Student.Emetic && !Student.Confessing && !StudentIsBusy && !Student.FocusOnYandere && !Student.Fleeing && !Student.Shoving && !Student.SentHome && Student.ClubActivityPhase < 16 && !Student.Vomiting && !Student.Lethal && !Student.Headache && !Student.Sedated && !Student.SenpaiWitnessingRivalDie && !Student.Hunted && !Student.Drowned && !Student.DramaticReaction && !Student.Yandere.Chased && !Student.Hunting && !Student.ImmuneToLaughter && !Student.ListeningToReport && !Student.Distracted && !Student.RetreivingMedicine && !Student.Struggling) || (!Student.StudentManager.Eighties && Student.Persona == PersonaType.Protective && Originator != null && Originator.StudentID == 11 && !Student.Hunted && !Student.Emetic && !Student.Headache && !Student.Slave && !Student.Sedated))
						{
	...
	private void InvestigateScream()
	{
		...
		if (!Student.YandereVisible && !Student.Distracted && !Student.Wet && !Student.Slave && !Student.WitnessedMurder && !Student.WitnessedCorpse && !Student.Headache && !Student.InEvent && !Student.Following && !Student.Confessing && !Student.Meeting && !Student.TurnOffRadio && !Student.Fleeing && !Student.Distracting && !Student.GoAway && !Student.FocusOnYandere && !StudentIsBusy && Student.Actions[Student.Phase] != StudentActionType.Teaching && Student.Actions[Student.Phase] != StudentActionType.SitAndTakeNotes && Student.Actions[Student.Phase] != StudentActionType.Graffiti && Student.Actions[Student.Phase] != StudentActionType.Bully && !Student.StudentManager.LockerRoomArea.bounds.Contains(base.transform.position))
		{
