...
public class PromptSwapScript : MonoBehaviour
{
	...
	public void UpdateSpriteType(InputDeviceType deviceType)
	{
		...
		if (joystickNames.Length != 0)
		{
			...
			{
				if (!joystickNames[i].ToLower().Contains("xbox") && !joystickNames[i].ToLower().Contains("8") && (joystickNames[i].ToLower().Contains("dual") || joystickNames[i].ToLower().Contains("less")))
				{
					flag = true;
				}
			}
