...
public class AchievementPopUpScript : MonoBehaviour
{
  ...
  private void Start()
  {
    this.transform.localPosition = new Vector3(786.5f, -553f, 0.0f);
  ...
  private void Update()
  {
    if (this.Show)
    {
      ...
      {
        this.transform.localPosition = Vector3.Lerp(this.transform.localPosition, new Vector3(this.transform.localPosition.x, -447.5f, 0.0f), Time.unscaledDeltaTime * 10f);
      }
      else
      {
        this.transform.localPosition = Vector3.MoveTowards(this.transform.localPosition, new Vector3(this.transform.localPosition.x, -553f, 0.0f), Time.unscaledDeltaTime * 100f);
        if (!(this.transform.localPosition == new Vector3(this.transform.localPosition.x, -553f, 0.0f)))
          return;
    ...
    else
    {
      ...
      for (int index = 1; index < this.AchievementNames.Length; ++index)
      {
        if (PlayerPrefs.GetInt(this.AchievementNames[index]) != this.PreviousNumber[index])
        {
          ...
          this.PreviousNumber[index] = PlayerPrefs.GetInt(this.AchievementNames[index]);
          this.Label.text = "Achievement Unlocked: " + this.AchievementFancyNames[index];
