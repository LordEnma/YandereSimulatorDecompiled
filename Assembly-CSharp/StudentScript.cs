...
public class StudentScript : MonoBehaviour
{
		...
		if (!Started)
		{
			...
			if (!StudentManager.Eighties)
			{
				...
				else if (StudentID == 51)
				{
					...
					if (ClubGlobals.GetClubClosed(ClubType.LightMusic))
					{
						...
						if (!SchoolGlobals.RoofFence)
						{
							...
							obj17.action = "Sulk";
							ScheduleBlock obj18 = ScheduleBlocks[7];
							obj18.destination = "Sulk";
							obj18.action = "Sulk";
							ScheduleBlock obj19 = ScheduleBlocks[8];
							obj19.destination = "Sulk";
							obj19.action = "Sulk";
						}
						else
						{
							ScheduleBlock obj20 = ScheduleBlocks[2];
							obj20.destination = "Seat";
							obj20.action = "Sit";
							ScheduleBlock obj21 = ScheduleBlocks[4];
							obj21.destination = "Seat";
							obj21.action = "Sit";
							ScheduleBlock obj22 = ScheduleBlocks[7];
							obj22.destination = "Seat";
							obj22.action = "Sit";
							ScheduleBlock obj23 = ScheduleBlocks[8];
				...
				else if (StudentID == 71)
				{
					...
					{
						PickPocket.gameObject.transform.parent.gameObject.SetActive(true);
					}
				}
				if ((StudentID == 6 || StudentID == 11) && DatingGlobals.SuitorProgress == 2)
				{
					...
					ScheduleBlock obj25 = ScheduleBlocks[2];
					obj25.destination = "Cuddle";
					obj25.action = "Cuddle";
					ScheduleBlock obj26 = ScheduleBlocks[4];
					obj26.destination = "Cuddle";
					obj26.action = "Cuddle";
					ScheduleBlock obj27 = ScheduleBlocks[6];
					obj27.destination = "Locker";
					obj27.action = "Shoes";
					ScheduleBlock obj28 = ScheduleBlocks[7];
			...
			else
			{
				...
				if (Rival)
				{
					...
					{
						ScheduleBlock obj29 = ScheduleBlocks[2];
				...
				else if (StudentID == 16)
				{
					...
					{
						ScheduleBlock obj30 = ScheduleBlocks[2];
			...
			if (!StudentManager.NoClubMeeting && Armband.activeInHierarchy && Clock.Weekday == 5)
			{
				if (StudentID < 86)
				{
					ScheduleBlock obj33 = ScheduleBlocks[6];
					obj33.destination = "Meeting";
					obj33.action = "Meeting";
				}
				else
				{
					ScheduleBlock obj34 = ScheduleBlocks[5];
	...
	public bool SeenByYandere()
	{
		...
		RaycastHit hitInfo;
		if (Physics.Linecast(Yandere.transform.position + new Vector3(0f, 1f, 0f), base.transform.position + new Vector3(0f, 1f, 0f), out hitInfo, YandereCheckMask))
		{
	...
	public bool CanSeeObject(GameObject obj, Vector3 targetPoint, int[] layers, int mask)
	{
		...
		if (PointIsInFOV(targetPoint))
		{
			...
			if (vector.sqrMagnitude <= num)
			{
				...
				RaycastHit hitInfo;
				if (Physics.Linecast(position, targetPoint, out hitInfo, mask) && (hitInfo.collider.gameObject == obj || hitInfo.collider.gameObject.transform.root.gameObject == obj))
				{
	...
	public bool CanSeeObject(GameObject obj, Vector3 targetPoint)
	{
		if (!Blind)
		{
			...
			RaycastHit hitInfo;
			if (num2 && flag && Physics.Linecast(position, targetPoint, out hitInfo, Mask) && hitInfo.collider.gameObject == obj)
			{
				return true;
			}
		}
	...
	private void UpdateRoutine()
	{
		if (Routine)
		{
			...
			else if (Phase < ScheduleBlocks.Length - 1 && Clock.HourTime >= ScheduleBlocks[Phase].time && !InEvent && !Meeting && ClubActivityPhase < 16 && !Ragdoll.Zs.activeInHierarchy)
			{
				...
				Phase++;
				BountyCollider.SetActive(false);
				if (Drownable)
				{
					Drownable = false;
					StudentManager.UpdateMe(StudentID);
				...
				{
					HorudaCollider.gameObject.SetActive(false);
				}
				BountyCollider.SetActive(false);
				if (StudentID == 81)
				{
				...
				if (Club == ClubType.Gardening && StudentID != 71 && Actions[Phase] == StudentActionType.ClubAction)
				{
					if (!StudentManager.Eighties)
					{
						if (WateringCan != null && WateringCan.transform.parent != Hips)
						{
				...
				if (Rival)
				{
					...
					{
						StudentManager.RivalBookBag.gameObject.SetActive(false);
			...
			if (CurrentDestination != null)
			{
				...
				{
					if (CurrentAction == StudentActionType.Follow && FollowTarget != null && FollowTarget.CurrentAction == StudentActionType.SearchPatrol)
					{
			...
			if (!Meeting)
			{
				if (!GoAway)
				{
					...
					else if (Actions[Phase] == StudentActionType.SitAndTakeNotes)
					{
						...
						{
							Debug.Log("Raibaru has been instructed to go sit down in her seat.");
							Follower.GoToClass();
						}
						if (MyPlate != null && MyPlate.parent == RightHand)
						{
						...
						if (Clock.Period != 2 && Clock.Period != 4)
						{
							...
							else
							{
								...
								if ((StudentID == 1 && StudentManager.Gift.activeInHierarchy) || (StudentID == 1 && StudentManager.Note.activeInHierarchy))
								{
									...
									{
										StudentManager.Gift.SetActive(false);
										StudentManager.Note.SetActive(false);
									}
								}
								else if (!Phoneless)
								{
					...
					else if (Actions[Phase] == StudentActionType.ClubAction)
					{
						...
						else if (Club == ClubType.LightMusic)
						{
							...
							{
								bool flag7 = (bool)InstrumentBag[ClubMemberID].transform.parent;
								if (!StudentManager.Eighties)
								{
									InstrumentBag[ClubMemberID].transform.position = new Vector3(0.5f, 4.5f, 22.45666f);
									InstrumentBag[ClubMemberID].transform.eulerAngles = new Vector3(-15f, 0f, 0f);
					...
					else if (Actions[Phase] == StudentActionType.Patrol)
					{
						...
						{
							StudentManager.MondayBento.SetActive(true);
							ExtraBento = false;
							if (Infatuated)
							{
					...
					else if (Actions[Phase] == StudentActionType.Read)
					{
						if (ReadPhase == 0)
						{
							if (StudentID == 5)
							{
								HorudaCollider.gameObject.SetActive(true);
							}
							CharacterAnimation.cullingType = AnimationCullingType.AlwaysAnimate;
							CharacterAnimation.CrossFade(BookSitAnim);
							if (CharacterAnimation[BookSitAnim].time > CharacterAnimation[BookSitAnim].length)
					...
					else if (Actions[Phase] == StudentActionType.Follow)
					{
						if (FollowTarget != null)
						{
							...
							if (FollowTarget.Routine && !FollowTarget.InEvent && FollowTarget.CurrentAction == StudentActionType.SitAndEatBento && FollowTarget.DistanceToDestination < 1f)
							{
					...
					else if (Actions[Phase] == StudentActionType.PrepareFood)
					{
						...
						{
							MyBento.Lid.SetActive(false);
							MyBento.Prompt.enabled = true;
							MyBento.transform.parent = null;
							MyBento.gameObject.SetActive(true);
						...
						if (ClubTimer > 60f)
						{
							...
							MyBento.gameObject.SetActive(false);
							MyBento.Prompt.enabled = false;
							MyBento.Prompt.Hide();
					...
					else if (Actions[Phase] == StudentActionType.PhotoShoot)
					{
						if (StudentID == 19)
						{
							Debug.Log("Yeah, the gravure model recognizes that she's supposed to be at a Photo Shoot...");
						}
			...
			if (MeetTimer == 0f)
			{
				...
				if (Yandere.Bloodiness + (float)Yandere.GloveBlood == 0f && (double)Yandere.Sanity >= 66.66666 && (CurrentDestination == StudentManager.MeetSpots.List[8] || CurrentDestination == StudentManager.MeetSpots.List[9] || CurrentDestination == StudentManager.MeetSpots.List[10]))
				{
					...
					else if (StudentID == 5)
					{
						...
						{
							StudentManager.FragileOfferHelp.gameObject.SetActive(true);
			...
			if (!Dying && !Spraying)
			{
				if (!PinningDown)
				{
					if (Persona == PersonaType.Dangerous)
					{
						...
						if (Yandere.Cauterizing || Yandere.Dismembering)
						{
		...
		if (Following && !Waiting)
		{
			...
			if (Phase < ScheduleBlocks.Length && (FollowCountdown.Sprite.fillAmount == 0f || Clock.HourTime >= ScheduleBlocks[Phase].time || StudentManager.LockerRoomArea.bounds.Contains(Yandere.transform.position) || StudentManager.WestBathroomArea.bounds.Contains(Yandere.transform.position) || StudentManager.EastBathroomArea.bounds.Contains(Yandere.transform.position) || StudentManager.IncineratorArea.bounds.Contains(Yandere.transform.position) || StudentManager.HeadmasterArea.bounds.Contains(Yandere.transform.position) || Yandere.Class.Portal.Transition || Yandere.TimeSkipping || Yandere.Trespassing))
			{
				...
				FollowCountdown.gameObject.SetActive(false);
				Pathfinding.canSearch = true;
				Pathfinding.canMove = true;
				...
				if (!Splashed)
				{
					if (!InDarkness)
					{
						...
						else if (BathePhase == 7)
						{
							...
							{
								StudentManager.CommunalLocker.Rings.gameObject.SetActive(false);
							}
						...
						else if (BathePhase == 9)
						{
							...
							else
							{
								...
								{
									Cosmetic.FemaleAccessories[Cosmetic.Accessory].SetActive(true);
								}
						...
						if (CharacterAnimation["f02_electrocution_00"].time > 2f && CharacterAnimation["f02_electrocution_00"].time < 5.9500003f)
						{
							...
							else
							{
								if (!LightSwitch.Flicker)
								{
									...
									GameObject obj19 = UnityEngine.Object.Instantiate(LightSwitch.Electricity, base.transform.position, Quaternion.identity);
									obj19.transform.parent = Bones[1].transform;
					...
					else if (DistanceToDestination < TargetDistance)
					{
						...
						if (DistractTimer <= 0f)
						{
							...
							if (StudentID == StudentManager.SuitorID && StudentManager.DatingMinigame.SuitorAndRivalTalking)
							{
								...
								Yandere.PromptParent.gameObject.SetActive(false);
							}
						}
					}
		...
		if (Hunting)
		{
			...
			if (HuntTimer > 1f)
			{
				GameObject obj20 = UnityEngine.Object.Instantiate(AlarmDisc, base.transform.position + Vector3.up, Quaternion.identity);
				obj20.GetComponent<AlarmDiscScript>().Originator = this;
				obj20.GetComponent<AlarmDiscScript>().Shocking = true;
				obj20.GetComponent<AlarmDiscScript>().NoScream = true;
				obj20.GetComponent<AlarmDiscScript>().Silent = true;
				HuntTimer = 0f;
			}
			if (HuntTarget != null)
			{
						...
						if (!NEStairs.bounds.Contains(base.transform.position) && !NWStairs.bounds.Contains(base.transform.position) && !SEStairs.bounds.Contains(base.transform.position) && !SWStairs.bounds.Contains(base.transform.position) && !NEStairs.bounds.Contains(HuntTarget.transform.position) && !NWStairs.bounds.Contains(HuntTarget.transform.position) && !SEStairs.bounds.Contains(HuntTarget.transform.position) && !SWStairs.bounds.Contains(HuntTarget.transform.position))
						{
							if (Pathfinding.canMove)
							{
								...
								if (!HuntTarget.Male)
								{
									...
									{
										HuntTarget.Instruments[HuntTarget.ClubMemberID].gameObject.SetActive(false);
									}
								}
								if (HuntTarget.Rival)
								{
							...
							else
							{
								...
								else if (MurderSuicidePhase == 2)
								{
									...
									{
										GameObject obj21 = UnityEngine.Object.Instantiate(Ragdoll.BloodPoolSpawner.BloodPool, base.transform.position + base.transform.up * 0.012f + base.transform.forward, Quaternion.identity);
										obj21.transform.localEulerAngles = new Vector3(90f, UnityEngine.Random.Range(0f, 360f), 0f);
		...
		if (Investigating)
		{
			...
			else if (InvestigationPhase == 100)
			{
				...
				{
					Debug.Log(Name + " reached InvestigationPhase 100.");
					StopInvestigating();
				}
			}
		}
		if (EndSearch)
		{
			...
			if (PatrolTimer > 5f)
			{
				...
				ScheduleBlock obj22 = ScheduleBlocks[4];
				obj22.destination = "LunchSpot";
				obj22.action = "Eat";
				ScheduleBlock obj23 = ScheduleBlocks[7];
				obj23.destination = "Hangout";
				obj23.action = "Hangout";
		...
		if (SeekingMedicine)
		{
			...
			else if (SeekMedicinePhase == 1)
			{
				...
				if (DistanceToDestination < 1f)
				{
					...
					Pathfinding.canMove = false;
					Pathfinding.speed = 0f;
					if (!studentScript3.Fleeing && !studentScript3.Guarding)
					{
						if (studentScript3.Investigating)
						{
							studentScript3.StopInvestigating();
						}
						StudentManager.UpdateStudents(studentScript3.StudentID);
			...
			else if (SeekMedicinePhase != 3)
			{
				...
				else if (SeekMedicinePhase == 5)
				{
					Pathfinding.canSearch = true;
					Pathfinding.canMove = true;
					ScheduleBlock obj24 = ScheduleBlocks[Phase];
					obj24.destination = "InfirmarySeat";
					obj24.action = "Relax";
					GetDestinations();
					CurrentDestination = Destinations[Phase];
					Pathfinding.target = Destinations[Phase];
				...
				if (Vector3.Distance(base.transform.position, new Vector3(BloodPool.position.x, base.transform.position.y, BloodPool.position.z)) < 1f)
				{
					...
					if (CharacterAnimation[InspectBloodAnim].time >= CharacterAnimation[InspectBloodAnim].length || Persona == PersonaType.Strict)
					{
						...
						if (WitnessedWeapon)
						{
							...
							if (!WitnessedBloodyWeapon && StudentID > 1 && !flag6 && CurrentAction != StudentActionType.SitAndTakeNotes && Indoors && !flag5 && Club != ClubType.Delinquent && !BloodPool.GetComponent<WeaponScript>().Dangerous && BloodPool.GetComponent<WeaponScript>().Returner == null && BloodPool.GetComponent<WeaponScript>().Origin != null)
							{
		...
		for (ID = 0; ID < Yandere.WeaponManager.Weapons.Length; ID++)
		{
			if (Yandere.WeaponManager.Weapons[ID] != null && (Yandere.WeaponManager.Weapons[ID].Blood.enabled || (Yandere.WeaponManager.Weapons[ID].Misplaced && Yandere.WeaponManager.Weapons[ID].transform.parent == null)))
			{
				...
				if (Yandere.WeaponManager.Weapons[ID].transform != LastSuspiciousObject && Yandere.WeaponManager.Weapons[ID].transform != LastSuspiciousObject2 && Yandere.WeaponManager.Weapons[ID].enabled)
				{
					...
					{
						if (!StudentManager.Eighties && StudentID == 48 && Yandere.WeaponManager.Weapons[ID].WeaponID == 12)
						{
							Debug.Log(Name + " could have reacted to a dropped dumbbell, but isn't going to.");
							break;
						}
						CharacterAnimation.cullingType = AnimationCullingType.AlwaysAnimate;
		...
		if (!flag)
		{
			...
			if (!WitnessedMurder && !CheckingNote && !Shoving && !Slave && !Struggling && flag2 && !Drownable && !Fighting)
			{
				...
				if (VisibleCorpses.Count == 0 && !WitnessedCorpse)
				{
					...
					else if ((StudentID == StudentManager.CurrentID || (Persona == PersonaType.Strict && Fleeing)) && !Wet && !Guarding && !IgnoreBlood && !InvestigatingPossibleDeath && !Spraying && !Emetic && !Threatened && !Sedated && !Headache && !SentHome && !Slave && !Talking && !Confessing && !SentToLocker && !Meeting && !IgnoringThingsOnGround && !RetreivingMedicine)
					{
						if (BloodPool == null && StudentManager.Police.LimbParent.childCount > 0)
						{
							UpdateVisibleLimbs();
						}
						if (BloodPool == null && (Police.BloodyWeapons > 0 || Yandere.WeaponManager.MisplacedWeapons > 0) && !InvestigatingPossibleLimb && !TakingOutTrash && !Alarmed && !InEvent && !InvestigatingPossibleBlood && !ChangingShoes && Persona != PersonaType.Violent && (MyPlate == null || (MyPlate != null && MyPlate.parent != RightHand)))
						{
									...
									else if (!Fleeing && (!Alarmed || CanStillNotice))
									{
										...
										if (IgnoreTimer == 0f || CanStillNotice)
										{
											...
											if (!Yandere.Chased && Yandere.Chasers == 0)
											{
												...
												if (Yandere.SuspiciousActionTimer > 0f || Yandere.PotentiallyAnnoyingTimer > 0f)
												{
	...
	private void UpdateTalkInput()
	{
		if (Prompt.Circle[0].fillAmount == 0f)
		{
			...
			if (!StudentManager.EmptyDemon)
			{
				...
				if ((Alarm > 0f || AlarmTimer > 0f || Yandere.Armed || Yandere.Shoved || Waiting || InEvent || SentHome || Threatened || Guarding || VisitSenpaiDesk || (Distracted && !Drownable) || StudentID == 1 || Yandere.YandereVision || TakingOutTrash) && ((!Slave && !BadTime && !Yandere.Gazing && !FightingSlave) || Yandere.YandereVision))
				{
			...
			if (Prompt.Circle[0].fillAmount == 0f)
			{
				...
				bool flag6 = false;
				bool flag7 = false;
				flag3 = StudentManager.Students[76] != null && StudentManager.Students[76].Friend;
				flag4 = StudentManager.Students[77] != null && StudentManager.Students[77].Friend;
				flag5 = StudentManager.Students[78] != null && StudentManager.Students[78].Friend;
				flag6 = StudentManager.Students[79] != null && StudentManager.Students[79].Friend;
				flag7 = StudentManager.Students[80] != null && StudentManager.Students[80].Friend;
				if (StudentID == 76 && GameGlobals.BlondeHair && Reputation.Reputation < -33.33333f && Yandere.Persona == YanderePersonaType.Tough && flag3 && flag4 && flag5 && flag6 && flag7)
				{
					flag2 = true;
					Warned = false;
				}
				...
				else if ((Clock.Period == 2 && !flag9) || (Clock.Period == 4 && !flag9) || (StudentID < 90 && CurrentDestination == Seat))
				{
				...
				else if (InEvent || !CanTalk || GoAway || Fleeing || (Meeting && !Drownable) || Wet || TurnOffRadio || InvestigatingBloodPool || (MyPlate != null && MyPlate.parent == RightHand) || flag8 || ReturningMisplacedWeapon || Actions[Phase] == StudentActionType.Bully || Actions[Phase] == StudentActionType.Graffiti || (CanTakeSnack && IgnoreFoodTimer > 0f) || (!StudentManager.Eighties && StudentID == 12) || (Rival && !Indoors) || (FollowTarget != null && FollowTarget.InEvent))
				{
				...
				else
				{
					...
					else
					{
						...
						if (!Grudge)
						{
							...
							else if (ClubGlobals.GetClubKicked(Club) && flag2)
							{
							...
							else if (Yandere.Club == Club && flag2 && ClubManager.ClubGrudge)
							{
						...
						else if (!Scrubber.activeInHierarchy && !flag11)
						{
							SmartPhone.SetActive(true);
						}
						ChalkDust.Stop();
		...
		if (flag12 || Yandere.Schoolwear == 2 || StudentManager.OriginalUniforms + StudentManager.NewUniforms > 1)
		{
			if (ClubActivityPhase >= 16)
			{
				return;
			...
			bool flag13 = false;
			if (Club == ClubType.Delinquent && !Injured && !Yandere.AttackManager.Stealth && !RespectEarned && !SolvingPuzzle && !Wet && MyWeapon != null && !Blind && !Yandere.Invisible)
			{
			...
			if (flag13 || Yandere.NearSenpai || Yandere.Attacking || Yandere.Stance.Current == StanceType.Crouching)
			{
				return;
			}
		...
		if (!Threatened)
		{
			...
			if (WitnessedCorpse)
			{
				if (!WalkBack)
				{
					...
					else if (!Phoneless && !Attacked)
					{
						if (Corpse.MurderSuicideAnimation)
						{
							Debug.Log("Waiting for murder suicide animation to end.");
							CharacterAnimation.CrossFade(ScaredAnim);
						}
						else
						{
							Debug.Log("Calling PhoneAddictCameraUpdate() from here, actually.");
							PhoneAddictCameraUpdate();
						}
					}
				}
			...
			else if (StudentID > 1)
			{
				...
				else
				{
					...
					if (FocusOnYandere)
					{
						if (DistanceToPlayer < 1f && !Injured && (Club == ClubType.Council || (Club == ClubType.Delinquent && !Injured) || Shovey))
						{
							AlarmTimer = 0f;
			...
			else if (AlarmTimer > 1f && !Reacted)
			{
				...
				else if (StudentID > 1 || Yandere.Mask != null)
				{
					...
					else if (Club != ClubType.Delinquent)
					{
						...
						{
							Debug.Log("Witnessed a weapon. WeaponWitnessed is: " + WeaponWitnessed);
			...
			if ((Club == ClubType.Council || Shovey) && DistanceToPlayer < 1.1f && !Yandere.Invisible && (Yandere.Armed || (Yandere.Carrying && !Yandere.CurrentRagdoll.Concealed) || (Yandere.Dragging && !Yandere.CurrentRagdoll.Concealed)) && Prompt.InSight)
			{
		...
		if (!Injured)
		{
			...
			if (!NoTalk)
			{
				...
				if (StudentManager.CombatMinigame.Delinquent == null)
				{
					...
					{
						StudentScript delinquent = StudentManager.CombatMinigame.Delinquent;
						Debug.Log("The game is trying to tell " + (((object)delinquent != null) ? delinquent.ToString() : null) + "to draw out a weapon.");
	...
	private void UpdateMisc()
	{
		...
		if (!Fleeing)
		{
			...
			if (Dying || Distracted || WalkBack || Waiting || Guarding || WitnessedMurder || WitnessedCorpse || Blind || SentHome || SentToLocker || TurnOffRadio || Wet || InvestigatingBloodPool || ReturningMisplacedWeapon || Yandere.Egg || StudentManager.Pose || ShoeRemoval.enabled || Drownable)
			{
				return;
			}
			...
			if ((Club == ClubType.Council || (Shovey && !Following)) && !WitnessedSomething)
			{
				if (DistanceToPlayer < 5f)
				{
					if (DistanceToPlayer < 2f)
			...
			if (((Club == ClubType.Council && !Spraying) || (Club == ClubType.Delinquent && !Injured && !RespectEarned && !Vomiting && !Emetic && !Headache && !Sedated && !Lethal) || (Shovey && !Spraying && !Following)) && (double)DistanceToPlayer < 0.5 && Yandere.CanMove && (Yandere.h != 0f || Yandere.v != 0f))
			{
				if (Club == ClubType.Delinquent)
				{
					Subtitle.Speaker = this;
	...
	public void SenpaiNoticed()
	{
		...
		Yandere.HeartRate.gameObject.SetActive(false);
		Yandere.Stance.Current = StanceType.Standing;
		Yandere.SneakShotPhone.SetActive(false);
		Yandere.ShoulderCamera.OverShoulder = false;
	...
	private void PersonaReaction()
	{
		...
		else if (Persona == PersonaType.Lovestruck)
		{
			...
			bool flag = false;
			if (!WitnessedMurder && Corpse != null && (Corpse.Student.Rival || (!StudentManager.Eighties && Corpse.StudentID == StudentManager.ObstacleID)))
			{
				flag = true;
			}
			if (flag)
	...
	public void GetDestinations()
	{
		...
		for (ID = 1; ID < JSON.Students[StudentID].ScheduleBlocks.Length; ID++)
		{
			...
			else if (scheduleBlock.destination == "Search Patrol")
			{
				...
				StudentManager.StolenPhoneSpot.gameObject.SetActive(true);
			...
			else if (scheduleBlock.destination == "InfirmaryBed")
			{
				if (Male)
				{
					Destinations[ID] = StudentManager.MaleRestSpots[StudentManager.SedatedStudents];
					StudentManager.SedatedStudents++;
				}
				else
				{
					Destinations[ID] = StudentManager.FemaleRestSpots[StudentManager.SedatedStudents];
					StudentManager.SedatedStudents++;
				}
			}
			...
			else if (scheduleBlock.action == "Patrol")
			{
				...
				if (OriginalClub != 0 && Club == ClubType.None)
				{
	...
	private void PickRandomGossipAnim()
	{
		...
		if (Actions[Phase] == StudentActionType.Gossip && DistanceToPlayer < 3f)
		{
			...
			if (!StudentManager.GetTopicLearnedByStudent(19, StudentID))
			{
				...
				StudentManager.SetTopicLearnedByStudent(19, StudentID, true);
			}
		}
	}
	...
	private void BecomeTeacher()
	{
		...
		if (StudentID > 89 && StudentID < 98 && StudentManager.Eighties)
		{
			...
			{
				EightiesTeacherAttacher.SetActive(true);
	...
	private void Nude()
	{
		...
		if (Club == ClubType.Cooking)
		{
			ApronAttacher.newRenderer.gameObject.SetActive(false);
	...
	public void ChangeSchoolwear()
	{
		...
		if (Club == ClubType.Cooking)
		{
			if (Schoolwear == 1)
			{
				ApronAttacher.newRenderer.gameObject.SetActive(true);
			}
			else
			{
				ApronAttacher.newRenderer.gameObject.SetActive(false);
		...
		if (Club == ClubType.LightMusic)
		{
			if (Schoolwear == 1)
			{
				InstrumentBag[ClubMemberID].gameObject.SetActive(true);
			}
			else
			{
				InstrumentBag[ClubMemberID].gameObject.SetActive(false);
			}
		}
		WalkAnim = OriginalWalkAnim;
	}
	...
	public void Spray()
	{
		...
		if (!flag)
		{
			if (!Yandere.Sprayed && !Dying && !Blind && !Yandere.Egg && !Yandere.Dumping && !Yandere.Bathing && !Yandere.Noticed && !Yandere.CannotBeSprayed)
			{
				...
				else
				{
					...
					DetectionMarker.gameObject.SetActive(false);
		...
		else
		{
			...
			StudentManager.Portal.SetActive(true);
	...
	public void GetFoodTarget()
	{
		...
		if (SleuthID < 90)
		{
			...
			{
				StudentActionType currentAction = StudentManager.Students[SleuthID].CurrentAction;
				int num = 10;
				StudentActionType currentAction2 = StudentManager.Students[SleuthID].CurrentAction;
				bool flag = StudentManager.Students[SleuthID].CurrentDestination == StudentManager.Exit;
				ClubType club = StudentManager.Students[SleuthID].Club;
				int num2 = 1;
				ClubType club2 = StudentManager.Students[SleuthID].Club;
				int num3 = 14;
				ClubType club3 = StudentManager.Students[SleuthID].Club;
				int num4 = 9;
				bool targetedForDistraction = StudentManager.Students[SleuthID].TargetedForDistraction;
				int clubActivityPhase = StudentManager.Students[SleuthID].ClubActivityPhase;
				int num5 = 16;
				bool inEvent = StudentManager.Students[SleuthID].InEvent;
				bool routine = StudentManager.Students[SleuthID].Routine;
				bool posing = StudentManager.Students[SleuthID].Posing;
				bool slave = StudentManager.Students[SleuthID].Slave;
				bool wet = StudentManager.Students[SleuthID].Wet;
				bool sedated = StudentManager.Students[SleuthID].Sedated;
				bool doNotFeed = StudentManager.Students[SleuthID].DoNotFeed;
				bool alreadyFed = StudentManager.Students[SleuthID].AlreadyFed;
				if (StudentManager.Students[SleuthID].Club == ClubType.LightMusic)
				{
					bool isPlaying = StudentManager.PracticeMusic.isPlaying;
				}
				GetFoodTarget();
			}
			else
	...
	public void DetermineWhatWasWitnessed()
	{
		...
		else if (YandereVisible)
		{
			...
			else if ((Yandere.Laughing && Yandere.LaughIntensity > 15f) || (StudentID > 1 && Yandere.Stance.Current == StanceType.Crouching) || Yandere.Stance.Current == StanceType.Crawling || Yandere.SuspiciousActionTimer > 0f || Yandere.WearingRaincoat || (Yandere.Carrying && Yandere.CurrentRagdoll.Concealed) || (Yandere.Dragging && Yandere.CurrentRagdoll.Concealed) || (Yandere.Schoolwear == 2 && Yandere.transform.position.z < 30f) || (AnnoyedByRadio > 1 && Yandere.PotentiallyAnnoyingTimer > 0f))
			{
	...
	{
		Debug.Log("A student cannot find the student they are supposed to be following.");
		CharacterAnimation.CrossFade(ParanoidAnim);
		SnackTimer += Time.deltaTime;
		if (SnackTimer > 5f)
		{
	...
	{
		Debug.Log(Name + " just put her bookbag on her desk.");
		...
		StudentManager.RivalBookBag.gameObject.SetActive(true);
		StudentManager.RivalBookBag.Prompt.enabled = true;
		StudentManager.RivalBookBag.Rival = this;
		BookBag.SetActive(false);
		if (GiftBox)
		{
			StudentManager.WednesdayGiftBox.SetActive(true);
			if (base.transform.position.x < 0f)
			{
		...
		if (VisitSenpaiDesk)
		{
			...
			{
				StudentManager.FridayTestNotes.SetActive(true);
		...
		else if (StudentID == 11 || StudentID == 12)
		{
			ScheduleBlock obj2 = ScheduleBlocks[2];
			obj2.destination = "Hangout";
			obj2.action = "Socialize";
		}
		else if (StudentID == 13)
		{
			ScheduleBlock obj3 = ScheduleBlocks[2];
		...
		else if (StudentID == 15)
		{
			...
			obj5.action = "Sunbathe";
		}
		else if (StudentID == 16)
		{
			ScheduleBlock obj6 = ScheduleBlocks[2];
			obj6.destination = "Perform";
			obj6.action = "Perform";
		}
	...
	{
		Debug.Log(Name + "fired the BecomeSleuth() function.");
